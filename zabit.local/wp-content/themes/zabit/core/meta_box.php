...
class MetaBox {
	...
	public function rss() {
	    ...
	    if ($post->post_type == $this->metabox['screen']) {
            foreach ($this->fields as $field) {
                ...
                if ((is_array($value) && count($value)) || (!is_array($value) && $value != '')) $field['value'] = $value;
	...
	public function display_number($args) {
		...
		?>
        <div class="components-base-control__field zabit-field zabit-field_number">
			<?php printf('<input %s />', implode(' ', array_filter(array(
				sprintf('%s="%s"', 'class', implode(' ', array_filter(array(
					'components-number-control__input',
					'zabit-field__input',
					'zabit-field__input_number',
					isset($args['class']) ? $args['class'] : null
				)))),
				isset($args['id']) ? sprintf('%s="%s"', 'id', $args['id']) : null,
				isset($args['name']) ? sprintf('%s="%s"', 'name', $args['name']) : null,
	...
	public function display_datetime($args) {
		...
		?>
        <div class="components-base-control__field zabit-field zabit-field_datetime">
            <?php if (in_array('mm', $args['options'])) : ?>
                <select id="<?php echo $id; ?>-mm" name="<?php echo $name; ?>[mm]">
                    <?php foreach (array('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec') as $i => $month) {
                        ...
                        printf('<option value="%s" %s>%s</option>', $num, selected($num, $mm, false), sprintf(__('%s-%s', 'zabit'), $num, $month));
                    } ?>
                </select>
	        <?php endif; ?>
			...
			<?php if (isset($args['description'])) : ?>
                <p class="description zabit-field__description"><?php echo $args['description']; ?></p>
			<?php endif; ?>
        </div>
		<?php
	}
	...
	public function display_checkbox($args) {
		$described = isset($args['description']) ? ''.$args['id'].'-description' : null;
		?>
        <div class="components-base-control__field zabit-field zabit-field_checkbox">
		<?php printf('<input %s />', implode(' ', array_filter(array(
			sprintf('%s="%s"', 'class', implode(' ', array_filter(array(
				'components-checkbox-control__input',
				'zabit-field__input',
				'zabit-field__input_checkbox',
				isset($args['class']) ? $args['class'] : null
			)))),
			isset($args['id']) ? sprintf('%s="%s"', 'id', $args['id']) : null,
			isset($args['name']) ? sprintf('%s="%s"', 'name', $args['name']) : null,
	...
	public function display_select($args) {
		$described = isset($args['description']) ? ''.$args['id'].'-description' : null;
		?>
		<div class="components-base-control__field zabit-field zabit-field_select">
	...
	public function display_media($args) {
		...
		?>
        <div class="components-base-control__field zabit-field zabit-field_media <?php echo $attachment ? 'zabit-field_value' : 'zabit-field_empty'; ?>" data-type="<?php echo $args['options']['type']; ?>">
			<?php printf('<input %s />', implode(' ', array_filter(array(
				sprintf('%s="%s"', 'class', implode(' ', array_filter(array(
					'components-hidden-control__input',
					'zabit-field__input',
	...
	public function display_gallery($args) {
		?>
        <div class="components-base-control__field zabit-field zabit-field_gallery" <?php echo implode(' ', array_filter(array(
            ...
            if (isset($args['value']) && $args['value'] != '' && count($args['value']) > 0) :
                ...
                foreach ($value as $v) :
                    ...
                    if (is_numeric($v)) :
	                    ...
	                    ?>
                        <span class="zabit-field__value">
                            <img class="zabit-field__value__img" src="<?php echo $attachment ? $attachment['src'] : ''; ?>" alt="<?php echo isset($args['placeholder']) ? $args['placeholder'] : '' ?>" />
                        </span>
                        <span class="zabit-field__actions">
                            <a class="zabit-field__action zabit-field__action_edit" href="#" title="<?php _e('Edit icon', 'zabit'); ?>"></a>
                            <a class="zabit-field__action zabit-field__action_remove" href="#" title="<?php _e('Remove icon', 'zabit'); ?>"></a>
            ...
            ?>
            <span class="zabit-field__actions">
                <a class="zabit-field__action zabit-field__action_add-image" href="#" title="<?php _e('Add icon', 'zabit'); ?>"></a>
                <a class="zabit-field__action zabit-field__action_add-url" href="#" title="<?php _e('Add url', 'zabit'); ?>"></a>
            </span>
        </div>
		<?php
	}
	...
	public function display_editor($args) {
		$described = isset($args['description']) ? ''.$args['id'].'-description' : null;
		?>
        <div class="components-base-control__field zabit-field zabit-field_editor">
	...
	public function rss2_editor($args) {
		if (isset($args['name']) && isset($args['value'])) {
			printf('<%1$s><![CDATA[%2$s]]></%1$s>', $args['name'], isset($args['value']) ? $args['value'] : '');
		}
	}

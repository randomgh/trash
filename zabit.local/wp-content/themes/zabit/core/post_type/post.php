...
function post_init(){
	foreach (array('post-formats', 'trackbacks', 'comments', 'author', 'custom-fields', 'page-attributes', 'excerpt') as $feature) {
		remove_post_type_support('post', $feature);
	}

	foreach (array('thumbnail') as $feature) {
...
add_action('rss2_item', 'post_item_rss');

function post_entity_rss() {
	global $post;
...
add_action('commentsrss2_head', 'post_entity_rss');
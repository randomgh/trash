...
function zabit_allowed_block_types($allowed_blocks, $post) {
...
add_filter('allowed_block_types', 'zabit_allowed_block_types', 10, 2);

function zabit_mce_init() {
	$type_prefix = 'zabit';
	...
	foreach (array('media', 'yt') as $block) {
		...
		if (file_exists($style_file_path)) {
			wp_register_style($block_id, $style_file_uri, array('zabit'), filemtime($style_file_path));
		...
		if (file_exists($script_file_path)) {
			wp_register_script($block_id, $script_file_uri, array('zabit'), filemtime($script_file_path));
...
add_action('init', 'zabit_mce_init');
